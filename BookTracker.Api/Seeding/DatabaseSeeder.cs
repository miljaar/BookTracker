...
public static class DatabaseSeeder
{
        ...
        IPasswordHasher<Member> passwordHasher)
    {
        ...
        if (settings is null || string.IsNullOrWhiteSpace(settings.Password))
            return;

        var email = new MemberEmail(settings.Email);
        var exists = dbContext.Members.Any(member => (string)member.Email == email);
        ...
        administrator.PasswordHash = passwordHasher.HashPassword(administrator, settings.Password);
